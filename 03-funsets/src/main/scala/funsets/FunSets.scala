...
trait FunSets extends FunSetsInterface:
  ...
  def forall(s: FunSet, p: Int => Boolean): Boolean =
    def iter(a: Int): Boolean =
      if contains(intersect(s, p), a) == false then false
      else if a == math.negateExact(bound) then contains(intersect(s, p), a)
      else iter(a - 1)
    iter(bound)
  ...
  def exists(s: FunSet, p: Int => Boolean): Boolean =
    def iter(a: Int): Boolean =
      if contains(intersect(s, p), a) == true then true
      else if a == math.negateExact(bound) then contains(intersect(s, p), a)
      else iter(a - 1)
    iter(bound)
